...
function SummaryDetails({ outputs, inputs, cache, sdk }) {
    ...
    <div class="summary__body">
        <article class="summary__block">
             <ul class="dim">
                ${inputs.selectedBroadbandPackage || inputs.selectedPhonePackage ? html`
                    ${ inputs.selectedBroadbandPackage ? html`<li>Broadband: ${ inputs.selectedBroadbandPackage.name }</li>` : '' }
                    ${ inputs.selectedTvPackage ? html`<li>TV: ${ inputs.selectedTvPackage.name }</li>` : '' }
        ...
        ${outputs.estimatedPriceBreakdown ? outputs.estimatedPriceBreakdown.map(section => html`
                <h4>${section.name}</h4>

                <article class="summary__block summary__block--bordered">
                    <table class="table">
                        ${section.contents.map(o => html`
                            <tr>
                                <th>${o.name}</th>
...
function SummaryPreview({ outputs, inputs }) {
    ...
    const selectedTvPackage = inputs.selectedTvPackage && inputs.selectedTvPackage.name;
    ...
    return html`
        ...
        ${inputs.selectedBroadbandPackage || inputs.selectedPhonePackage ? html`
            <span class="faint summary__preview-info">
                ...
                ${ selectedTvPackage ? html`<span>TV: ${ selectedTvPackage }</span>` : '' }
...
function hasContent(inputs) {
    return inputs.selectedBroadbandPackage || inputs.selectedTvPackage || inputs.selectedPhonePackage;
}
