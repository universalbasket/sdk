...
(function (factory) {
  ...
  class EndUserSdk {
    ...
    async create({
      ...
      set('_', 'serviceName', job.serviceName);
      set('_', 'otp', otp);
    }
...
`;
  ...
  class PageRenderer {
    ...
    addListeners(name) {
      ...
      const vaultListener = () => {
        ...
        this.waitForVaultOutput().then(({
          cardToken,
          panToken
        }) => {
          submitBtn.setAttribute('disabled', 'true');
          const inputs = serializeForm(`#section-form-${name}`);
          if (inputs.payment) inputs.payment['card'] = {
            '$token': cardToken
          };
          inputs['panToken'] = panToken;
  ...
  var NotFound = selector => ({
    render: () => render(notFount404(), document.querySelector(selector))
  });
...
<div class="summary">
  ...
  var ErrorTemplate = (selector = '#app') => {
    return {
      init: () => {
        const target = document.querySelector(selector);
...
`;
  ...
  function createApp({
    ...
    const routes = {
      ...
      '/error': {
        renderer: ErrorTemplate(mainSelector),
        title: null,
        step: null
      }
    };
    pages.forEach((config, idx) => {
      ...
      const renderer = getPageRenderer(name, sections, mainSelector, onFinish);
      routes[route] = {
        renderer,
        title,
        step: idx + 1
...
`;
  ...
  const baseUrl = 'https://vault.automationcloud.net/forms/index.html'; //check available options: https://docs.automationcloud.net/docs/vaulting-payment-card#section-styling-and-configuring-the-form
...
`;
  ...
  var finalPriceConsent = finalPrice => {
    const {
      value,
      currencyCode
...
`;
  ...
  var installation = (installationOptions = {}) => html`
...
`;
  ...
  var selectedBroadbandSetupDate = availableDates => html`
    <div class="field">
    ...
    <div class="field">
        ...
        <div class="field__inputs group group--merged">
            <input type="radio" name="monthly-payment-method" id="monthly-payment-method-card"
                value="card" @click="${onChange$1}" required checked>
            ...
            <input type="radio" name="monthly-payment-method" id="monthly-payment-method-directdebit"
                value="directdebit" @click="${onChange$1}"required>
...
`;
  ...
  var finalPriceConsent$1 = (name, {
    oneOffCosts,
    finalPrice
  }) => html`${finalPriceConsent(finalPrice)}
        <div class="section__actions">
...
`;
  ...
  Object.freeze({
    ...
    checkout: checkout,
    finalPriceConsent: finalPriceConsent$1,
